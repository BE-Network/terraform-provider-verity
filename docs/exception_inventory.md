...
| Singleton members are omitted when null | 38 fields | A null member of `object_properties` is dropped from the request rather than sent as a zero value, which is what the handwritten resources do. The API merges a PATCHed object member by member, so an omitted member is left unchanged, not cleared; a member is cleared by writing its empty value. Required adding `UpdateClearOmit`, which the Phase 0 vocabulary lacked. |
