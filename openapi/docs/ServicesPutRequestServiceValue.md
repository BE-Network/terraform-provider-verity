...
**Vni** | Pointer to **NullableInt64** | Identifies the service within the VXLAN fabric | [optional] 
...
**Mtu** | Pointer to **NullableInt64** | MTU (Maximum Transmission Unit) The size used by a switch to determine when large packets must be broken up into smaller packets for delivery. If mismatched within a single vlan network, can cause dropped packets. | [optional] [default to 1500]
