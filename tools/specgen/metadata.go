...
// renderResourceKeys emits the API keys each resource implementation needs. They
// are currently repeated as per-resource constants and inline literals, which is
// the duplication the plan identifies: the same endpoint name and cache key are
// written in the resource, in the registry, and in the bulk configuration.
func renderResourceKeys(resources spec.Registry) ([]byte, error) {
	...
	buf.WriteString("// generatedResourceKey holds the API names one resource addresses: the endpoint\n")
	buf.WriteString("// it lives at, the provider cache it shares with its siblings, and the key its\n")
	buf.WriteString("// objects appear under in a GET response.\n")
	...
	buf.WriteString("// generatedResourceOrder is the canonical registration order: every API-backed\n")
	buf.WriteString("// resource in the registry, by Terraform name. The provider enumerates this\n")
	buf.WriteString("// rather than a handwritten list, so adding a resource to the registry is what\n")
	buf.WriteString("// registers it.\n")
